"""Round-trip tests for the prolance Python bindings.

These build a tiny in-memory mzML, write it to disk, ingest into a
ProLance store, and exercise indexing + range queries.
"""
...
def test_ingest_index_query_roundtrip(tmp_path):
    ...
    store = prolance.Store.open(str(tmp_path / "store"))
...
def test_store_repr(tmp_path):
    store = prolance.Store.open(str(tmp_path / "store"))
    assert "prolance.Store" in repr(store)
