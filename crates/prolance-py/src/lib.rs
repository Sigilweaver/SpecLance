...
#[pyclass(module = "prolance._prolance", name = "Store")]
struct PyStore {
    rt: Arc<Runtime>,
    inner: Arc<CoreStore>,
    path: String,
...
#[pymethods]
impl PyStore {
    /// Open (or create) a ProLance store at `path`.
    #[staticmethod]
    ...
    fn __repr__(&self) -> String {
        format!("<prolance.Store path={:?}>", self.path)
    }
        ...
        let batches = self
            ...
            .block_on(async move {
                ...
                if !names.iter().any(|n| n == "runs") {
                    return Ok::<_, prolance_core::Error>(Vec::new());
                }
...
#[pymodule]
fn _prolance(m: &Bound<'_, PyModule>) -> PyResult<()> {
